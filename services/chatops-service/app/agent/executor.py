    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                ...
                "1) 先在 user-service 日志中根据用户名（例如 user=peter）筛选登录或请求日志，可以结合 login/signin 等关键字；此时只能使用用户名过滤，禁止再额外增加 user_id 关键字，尤其禁止写成 {{app=\"user-service\"}} |= \"user=peter\" |= \"user_id\" 这种同时要求两者命中的表达式，因为很多登录日志只包含用户名、不包含 user_id。"
                ...
                "3) 再在 todo-service 和 ai-service 日志中，使用步骤 2 中得到的 user_id 作为关键字（例如 |= \"user_id=22\"）过滤出与该用户相关的待办操作和 AI 对话请求，此时不要再额外要求日志中必须出现用户名。"
